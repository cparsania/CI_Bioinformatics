...
cran_pkgs <- c("tidyverse", "pheatmap", "RColorBrewer")
bioc_pkgs <- c("DESeq2", "EnhancedVolcano", "biomaRt", "clusterProfiler", "org.Hs.eg.db")

# Install CRAN packages (skip if already installed)
install_cran <- cran_pkgs[!cran_pkgs %in% rownames(installed.packages())]
if (length(install_cran) > 0) install.packages(install_cran)

# Install Bioconductor packages (skip if already installed)
install_bioc <- bioc_pkgs[!bioc_pkgs %in% rownames(installed.packages())]
if (length(install_bioc) > 0) BiocManager::install(install_bioc)
...
ego_down <- enrichGO(gene = down_genes, 
                     ...
                     minGSSize = 10, 
                     maxGSSize =500,
                     universe = res$entrezgene_id %>% as.character() %>% unique())
